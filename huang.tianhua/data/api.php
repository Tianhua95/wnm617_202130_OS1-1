...
function makeStatement($data) {
   ...
   switch($t) {
      ...
      case "locations_all":
         return makeQuery($c,"SELECT * FROM `track_202103_locations`",$p);
      ...
      case "location_by_id":
         return makeQuery($c,"SELECT * FROM `track_202103_locations` WHERE id=?",$p);


      case "animals_by_user_id":
         return makeQuery($c,"SELECT * FROM `track_202130_animals` WHERE user_id=?",$p);
      case "locations_by_animal_id":
         return makeQuery($c,"SELECT * FROM `track_202103_locations` WHERE animal_id=?",$p);
      ...
      case "insert_location":
         $r = makeQuery($c,"INSERT INTO
            `track_202103_locations`
            (`animal_id`,`lat`,`lng`,`description`,`photo`,`icon`,`date_create`)
            VALUES
            (?,?,?,?,'https://via.placeholder.com/500/?text=Photo','img/location.svg', NOW())
            ",$p,false);
         return ["id"=>$c->lastInsertId()];
      ...
      case "delete_location":
         return makeQuery($c,"DELETE FROM `track_202103_locations` WHERE `id` = ?",$p,false);
